...
#define mq135 A0        //mq135
DHT dht(DHTPIN, DHTTYPE);

// static char telemetry_payload[128];  // Assuming a fixed-size payload

static char* getTelemetryPayload(int temperature,int humidity) {

    // Create a span for the telemetry payload buffer
    az_span temp_span = az_span_create(telemetry_payload, sizeof(telemetry_payload));

    // Copy static part of the telemetry payload
    temp_span = az_span_copy(temp_span, AZ_SPAN_FROM_STR("{ \"messageID\": "));
...
static void sendTelemetry(int temperature,int humidity)
{
